...
function SubmitButton({ enabled }: { enabled: boolean }) {
  ...
  return (
    <button
      type="submit"
      className={`w-full rounded-md px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-offset-2 flex items-center justify-center ${
        enabled
    ...
    >
      {pending ? (
        <>
          ...
          {getTranslation(language, 'login.loggingIn')}
        </>
      ) : (
